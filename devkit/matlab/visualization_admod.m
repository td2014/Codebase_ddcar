function varargout = visualization_admod(mode,image_dir,varargin)
...
switch mode
  ...
  case 'update'
    ...
    % read image
    if (img_idx < nimages-1)
       
       img = imread(sprintf('%s/%010d.png',image_dir,img_idx));
       img_d = imread(sprintf('%s/%010d.png',image_dir,img_idx+1));
       thresh = 75;
    
    % clear axes, draw image
       cla(h(1).axes); cla(h(2).axes);
       imshow((img_d-img)>thresh);
     endif
